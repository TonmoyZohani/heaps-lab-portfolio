...
import { ChevronDown, Menu, X, ArrowRight, Code2, Palette, Megaphone, ArrowUpRight, Sparkles } from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <>
      {/* ── Navbar bar — always exactly 68px tall ── */}
      <header
        className={cn(
          "fixed top-0 left-0 right-0 z-50 h-[68px] flex items-center justify-between px-6 md:px-10 lg:px-16 transition-all duration-300",
          isWhite ? "bg-white" : "bg-transparent",
          scrolled ? "shadow-[0_1px_0_0_rgba(0,0,0,0.06)]" : "shadow-none"
        )}
      ...
      >
            ...
            if ("mega" in item && item.mega) {
              return (
                <li key={item.label} onMouseEnter={() => setMegaOpen(true)}>
                  <button
                    className={cn(
                      "flex items-center gap-1 text-[1rem] font-bold px-4 py-1.5 rounded-full transition-all duration-200",
                      megaOpen
            ...
            return (
              <li key={item.label} onMouseEnter={() => setMegaOpen(false)}>
                <Link
                  href={item.href}
                  className={cn(
                    "text-[1rem] font-bold px-4 py-1.5 rounded-full transition-all duration-200",
                    isWhite
                      ? "text-foreground/70 hover:bg-[#E01F59] hover:text-white"
                      : "text-white/80 hover:bg-[#E01F59] hover:text-white"
                  )}
        ...
        {/* Mobile hamburger */}
        <button
          className={cn("lg:hidden p-2 transition-colors", isWhite ? "text-foreground" : "text-white")}
      ...
      {/* Mobile drawer — positioned below the fixed header */}
      {mobileOpen && (
        <div className="fixed top-[68px] left-0 right-0 z-40 bg-background border-t border-black/5 lg:hidden max-h-[80vh] overflow-y-auto shadow-xl">
      ...
      {/* ── Mega menu panel — separate fixed element, sits below the 68px header ── */}
      <div
        className={cn(
          "fixed top-[68px] left-0 right-0 z-40 transition-all duration-300 ease-out",
          megaOpen
              ...
              <div className="relative border-r border-black/[0.06] px-7 py-8">
                <div className="flex flex-col h-full">
                  ...
                  <div className="mt-6 space-y-0.5">
                    {[
                      { label: "Development", href: "/services/development", icon: Code2, description: "Web, mobile & custom software" },
                      { label: "Design", href: "/services/design", icon: Palette, description: "UI/UX, brand & visual identity" },
                      { label: "Marketing", href: "/services/marketing", icon: Megaphone, description: "Growth & digital marketing" },
                      ...
                      >
                        ...
                        <span className="min-w-0 flex-1">
                          <span className="block text-[13px] font-bold text-foreground/70 transition-colors group-hover:text-foreground leading-tight">{label}</span>
                          <span className="block mt-0.5 text-[11px] text-foreground/35 leading-tight">{description}</span>
                        </span>
                  ...
                  >
                    ...
                    <div className="relative flex items-end justify-between">
                      <div>
                        <p className="text-[9px] font-bold uppercase tracking-[0.18em] text-white/55">Have a project?</p>
                        <p className="mt-1 text-sm font-semibold text-white">Let&apos;s build it.</p>
                      </div>
                            ...
                            <li key={svc.href}>
                              ...
                              >
                                <span className="flex items-center gap-3">
                                  ...
                                  <span className="text-[15px] font-semibold text-foreground/60 transition-colors group-hover/item:text-foreground">{svc.label}</span>
                                </span>
                ...
                <div className="mt-9 overflow-hidden rounded-2xl border border-black/[0.06] bg-[#FAFAFA]">
                  <div className="flex items-center justify-between px-5 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-xs font-semibold text-foreground/70">Need something custom?</p>
                        <p className="mt-0.5 text-[11px] text-foreground/35">Tell us what you&apos;re building and we&apos;ll figure it out together.</p>
                      </div>
                    </div>
                    <Link
                      href="/contact"
